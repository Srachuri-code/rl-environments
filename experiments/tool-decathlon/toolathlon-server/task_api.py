...
Key optimization for RL: 
- /setup: Full setup with MCP server initialization (slow, ~30-60s)
- /reset: Fast workspace-only reset, keeps MCPs alive (fast, ~1s)

Use /reset when MCPs are already connected for the same server set.
"""
...
class TaskAPI:
    ...
    async def reset(
        ...
        """
        Fast reset: keep MCP connections alive, just reset workspace.
        
        This is the KEY OPTIMIZATION for RL training:
        - MCPs stay connected (no 30-60s init overhead)
        - Just clear workspace and copy task files (~1s)
        
        Use when the same MCP servers are needed across episodes.
        """
        print(f"[RESET] Fast reset for task: {task_id}", flush=True)
        
        async with self._setup_lock:
            self.task_id = task_id
            new_workspace = f"/toolathlon/agent_workspace/{task_id}"
            ...
            task_dir = f"/toolathlon/tasks/finalpool/{task_id}"
            workspace_src = Path(task_dir) / "workspace"
            if workspace_src.exists():
                shutil.copytree(workspace_src, new_workspace, dirs_exist_ok=True)
            
            self.workspace = new_workspace
            
            # Reset context and messages
            self.context = MinimalAgentContext(self.workspace, [])
            self.messages = []
            
            # Update MCP manager's workspace reference
            if self.mcp_manager:
                self.mcp_manager.agent_workspace = self.workspace
            
            # Collect tools from connected MCPs (already alive)
            all_tools = []
            if self.mcp_manager:
                for server_name in self.mcp_manager.get_connected_server_names():
                    server = self.mcp_manager.connected_servers[server_name]
            ...
            # Add local tools based on capabilities
            needed_local_tools = local_tools or []
            if not needed_local_tools:
                # Try to read from task config
                try:
                    task_config = read_json(f"{task_dir}/task_config.json")
            ...
            def _tool_to_oai(tool_obj) -> dict[str, Any]:
                return {
                    ...
                    "function": {
                        ...
                        "parameters": getattr(tool_obj, "params_json_schema", None) or {"type": "object", "properties": {}},
                    },
                }
            ...
            print(f"[RESET] Reset complete with {len(all_tools)} tools", flush=True)
            ...
            return {
                ...
                "workspace": self.workspace,
                "tools": all_tools,
                "mcps_connected": list(self.mcp_manager.get_connected_server_names()) if self.mcp_manager else [],
            }
    
    def get_connected_mcps(self) -> list[str]:
        """Return list of currently connected MCP servers."""
        if self.mcp_manager:
            return list(self.mcp_manager.get_connected_server_names())
        return []
...
async def serve(host: str, port: int):
    ...
    async def reset(payload: dict[str, Any]):
        """
        Fast reset: keep MCPs alive, just reset workspace.
        
        Use this when running multiple episodes with the same MCP servers.
        Much faster than /setup (~1s vs ~30-60s).
        """
        task_id = payload.get("task_id")
        if not task_id:
            return JSONResponse({"error": "missing task_id"}, status_code=400)
        ...
        result = await _api.reset(str(task_id), local_tools)
        return JSONResponse(result)

    @app.get("/mcps")
    async def get_mcps():
        """Return list of currently connected MCP servers."""
        return JSONResponse({"connected_mcps": _api.get_connected_mcps()})
